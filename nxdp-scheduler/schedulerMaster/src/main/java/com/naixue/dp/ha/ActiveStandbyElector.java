...
/**
 * Created by sunzhiwei on 2018/1/22.
 */
public class ActiveStandbyElector {
    ...
    private static final String MASTER_HA_PATH = "/master";
...
//            hostname = "testHost";
            ...
            EnsurePath ensurePath = zkClinet.newNamespaceAwareEnsurePath(SCHEDULER_HA_PATH + MASTER_HA_PATH
                    + "/" + hostname);
...
//            zkClinet.create().forPath("/szw/s", new byte[0]);
    ...
    private boolean getHaHostData(){
        ...
        try {
            List<String> hostList = zkClinet.getChildren().forPath(SCHEDULER_HA_PATH + MASTER_HA_PATH);
            for (String host : hostList){
                if (!host.contains("lock")){
                    String data = new String(zkClinet.getData().forPath(SCHEDULER_HA_PATH + MASTER_HA_PATH + "/" + host));
    ...
    private void deleteHostData(String hostname){
        Stat stat = null;
        try {
            stat = zkClinet.checkExists().forPath(this.SCHEDULER_HA_PATH + MASTER_HA_PATH + "/" + hostname);
            if (stat != null){
                zkClinet.delete().forPath(this.SCHEDULER_HA_PATH + MASTER_HA_PATH + "/" + hostname);
            }
        } catch (Exception e) {
    ...
    public void updateHostData(String host, boolean isActive){
        ...
        Stat stat = null;
        try {
            stat = zkClinet.checkExists().forPath(SCHEDULER_HA_PATH + MASTER_HA_PATH + "/" + host);
            if (stat == null){
                logger.info("create a new host : " + host);
                zkClinet.create().forPath(SCHEDULER_HA_PATH + MASTER_HA_PATH + "/" + host, data.getBytes());
            } else {
                logger.info("update data of host : " + host);
                zkClinet.setData().forPath(SCHEDULER_HA_PATH + MASTER_HA_PATH + "/" + host, data.getBytes());
            }
        } catch (Exception e) {
...
//            }// 如果只判断另一个节点的状态，是不是localStauts的状态就无所谓了？这是重复代码呀
    ...
    public boolean isActive(){
        ...
        // 分布式重入锁
        InterProcessMutex nodeLock = new InterProcessMutex(this.zkClinet, SCHEDULER_HA_PATH + MASTER_HA_PATH);
        try {

...
class SparkInterpreter(InterpreterBase):

    # 'Interpreter of SPARK'
    def interpret(self, job_id, query_name, namespace, choose_time):
        ...
        if update_command["code"] == 1:
            log_util.logger.error("[SparkInterpreter] update dispatch_command error with return %s", update_command["desc"])
            update_command["desc"] = "[SparkInterpreter] update dispatch_command error with return %s" % update_command["desc"]
            return json.dumps(update_command)

        prepare_params = self.prepare_interp_env(job_id)
        log_util.logger.debug("[SparkInterpreter] %s", prepare_params)

        if prepare_params["code"] == 1:
            log_util.logger.debug("[SparkInterpreter] error: %s", prepare_params["desc"])
        ...
        log_util.logger.debug("[SparkInterpreter] tmp_script_folder_path: %s", tmp_script_folder_path)
        ...
        log_util.logger.debug("[SparkInterpreter] get_to_local: %s", get2local_str)
...
'''
        ...
        log_util.logger.debug("[SparkInterpreter] scripts path: %s", tmp_script_path)
...
'''
    ...
    def parse_dispatch_command(self, job_id, choose_time):
        ...
        if details["code"] == 1:
            log_util.logger.error("[SparkInterpreter] %s", details["desc"])
            return details
...
if __name__ == "__main__":
    ...
    print spark.interpret(34504, "1252", "2018-05-22", "2018-05-22")

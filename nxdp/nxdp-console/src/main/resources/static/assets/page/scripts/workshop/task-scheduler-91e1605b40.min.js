var TaskScheduler = (function(){
    ...
    var initStatus = function() {
  	  ...
  	  $('#schedulerStatus').change(function() {
  	    console.log('当前任务的按钮被置为（1：上线、2：下线）：'+$('#schedulerStatus').attr('value'));
  		if($(this).prop('checked')) {
  	    	...
  	    	if(isNullOrEmpty($('#hidId').val())) {
  	    		...
  	    		if(confirmNoExistJobRelation()){
  	    			...
  	    			bootbox.confirm('存在其他上线任务依赖于当前任务。确定要下线当前任务？（编辑页出现该弹框表示在数据库中当前任务实际已经下线了）', function(event) {
  	  	  	            if(event) {
    ...
    var confirmNoExistJobRelation = function() {
      ...
      $.ajax({
        type: "POST",
        url:  self.contextPath + "/old-page/dev/task/checkJobHaveChildsByDevId",
        async:false,
        data: {"id": devId},
        dataType: 'json',
      ...
      console.log('该任务是否不被其他任务依赖？'+flag);
      return flag;
    }
    ...
    var initHadoopQueueList = function() {
        ...
        $.post(self.contextPath + '/hadoop/api/getHadoopQueueByDept', function(data, status) {
            ...
            _.each(data, function(queue){
                ...
                //筛选
                if(jobType == 16 && queue.name.indexOf("spark") == -1){
                  return true;
                }
